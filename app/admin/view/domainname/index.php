...
<script type="text/html" title="操作按钮模板" id="buttonTpl">
    <a href="{:url('admin/Domainname/wechat')}?id={{ d.id }}" class="layui-btn layui-btn-xs layui-btn-normal">微信号管理</a>
    <a href="{:url('admin/Domainname/edit')}?id={{ d.id }}" class="layui-btn layui-btn-xs layui-btn-normal">修改</a>
    <a href="{:url('admin/Domainname/del')}?id={{ d.id }}" class="layui-btn layui-btn-xs layui-btn-danger j-tr-del">删除</a>
</script>
<script type="text/javascript">
    ...
    layui.use(['table'], function() {
        ...
        table.render({
            elem: '#dataTable'
            ,url: "{:url('admin/Domainname/index')}?search="+search //数据接口
            ,page: true //开启分页
            ,limit: 10
            ,search:search
            ,text: {
