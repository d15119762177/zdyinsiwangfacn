<div class="page-toolbar">
    <div class="layui-btn-group fl">
        ...
        <a data-href="{:url('admin/Agent/status?val=1')}" class="layui-btn layui-btn-primary j-page-btns layui-icon layui-icon-play" data-table="dataTable">&nbsp;开启</a>
        <a data-href="{:url('admin/Agent/status?val=0')}" class="layui-btn layui-btn-primary j-page-btns layui-icon layui-icon-pause" data-table="dataTable">&nbsp;关闭</a>
        <a data-href="{:url('admin/Agent/del')}" class="layui-btn layui-btn-primary j-page-btns confirm layui-icon layui-icon-close red">&nbsp;删除</a>
    </div>
    <div class="page-filter fr">
        <form class="layui-form layui-form-pane" action="{:url('admin/Agent/index')}" method="get">
...
<script type="text/html" title="操作按钮模板" id="buttonTpl">
    <a class="layui-btn layui-btn-xs layui-btn-normal" onclick="tankuang({{ d.id }})" >佣金列表</a>
    <a href="{:url('admin/Agent/edit')}?id={{ d.id }}" class="layui-btn layui-btn-xs layui-btn-normal">修改</a>
    <a href="{:url('admin/Agent/del')}?id={{ d.id }}" class="layui-btn layui-btn-xs layui-btn-danger j-tr-del">删除</a>
</script>
<script type="text/javascript">
    ...
    layui.use(['table'], function() {
        ...
        table.render({
            elem: '#dataTable'
            ,url: "{:url('admin/Agent/index')}?search="+search //数据接口
            ,page: true //开启分页
            ,limit: 10
            ,search:search
            ,text: {
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'waitCommision', title: '待结算佣金',  width: '10%',sort: true}
                ,{field: 'finishCommision', title: '已结算佣金', width: '10%',sort: true}
...
<script>
    ...
    function tankuang(obj)
    {
        layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: "{:url('admin/agent/list_log')}?id="+obj
        });
    }
